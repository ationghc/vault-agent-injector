...
**The next step is to update the deployment nginx-deployment with vault-agent-injector annotations, using kubectl replace.**

CLI: Deletes deployment then recreates it with an updated version
```
kubectl  replace -f nginx-deployment-annotated.yaml
```
**kubectl replace command**
* Write operations are atomic
* Updates cannot occur at the same time. 
* Complete resource spec must be provided in a YAML or JSON file
 
**Step 7.**

**The next step is to confirm the annotations were applied to the deployment object.**

CLI: Checks deployment pod spec annotations
```
kubectl get deployment nginx-deployment -o json | jq -r .spec.template.metadata.annotations
```
...
**The next step is to confirm if the containers were injected into the nginx application pod after updating the deployment annotations.**
...
```
```
The output shows additional containers were injected into the application pod. But none of the containers have a healthy status. 
- The Ready column shows `0 of 2` container ready
- The Status column shows `0 of 1 initContainers ready
```
```
NAME                               READY   STATUS     RESTARTS   AGE
...
**The next step is to check container logs for clues:**
* Check logs of vault-agent-injector pod
* Check logs of initContainer 
* Check logs of K8s API
 
```
CLI: Writes vault agent injector container logs to standard output
kubectl logs vault-agent-injector-8496df644f-kfnbb -f

CLI: Writes application pod initContainer logs to standard output
kubectl logs nginx-deployment-585f4cdbf-6p4pw -c vault-agent-init -f

CLI:Writes Kubernetes API logs to standard output and search for the string inject
...
2023-07-28T18:28:21.751Z [ERROR] auth.handler: error authenticating:
  ...
  | * invalid role name "test-app"
   backoff=4m25.27s
```
**The error above means that the role defined in the deployments annotation doesn't exist.**
